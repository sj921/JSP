...
public class AddCookieServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		cookie4.setMaxAge(60 * 60 * 24 * 365); // keep 365 days
				
		cookie3.setPath(req.getContextPath() + "/search");
		cookie.setMaxAge(-1);	// session, not stored
		...
		// 쿠키의 경로 (해당 쿠키가 적용되는 uri를 설정)
		cookie.setPath(req.getContextPath()); // = cookie.setPath("/")		
		// 쿠키의 수명 설정 (설정하지 않으면 세션 만료시까지 유지됨, 초 단위)
		cookie.setMaxAge(30);	
		// 쿠키의 값 설정
