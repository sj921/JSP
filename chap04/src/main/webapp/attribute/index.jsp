...
<body>
	...
	<ul>
		<li>
			...
			attribute는 서버에서 활용하기 위해 개발자가 등록해놓은 값이다
		</li>
		<li>
			각 attribute 종류별로 유효한 범위(Scope)가 다르면 용도에 맞게 활용해야 한다
	...
	<dl>
		...
		<dd>
			...
			사용자가 서버에 처음 접속하면 서버로부터 세선ID를 부여받는다.
			그 세션ID는 유일한 값이며 클라이언트 본인을 증명할 수 있는 유일한 수단이 된다.
			세션은 설정된 시간이 모두 지나거나 (은행 사이트)
			웹 브라우저를 종료하면 만료된다.
		</dd>
		<dt>application scope</dt>
		<dd>
			웹 어플리케이션 서버에 저장해놓는 어트리뷰트. (WAS)
			서버에 직접 저장되며 서버가 종료될 때까지 유지된다.
			세션 객체는 접속자 수만큼 존재하지만 서버는 1대이므로 전역 변수라고 볼 수 있다.
		</dd>
	</dl>
	...
	<button id="test1">어트리뷰트 등록하고 포워드하기</button>
